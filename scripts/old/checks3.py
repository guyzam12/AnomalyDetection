...
def get_mse(x, y):
    #y_temp = y.loc[y<y.quantile(0.90)]
    #idx = y_temp.index.values
    #x_temp = x.loc[idx]
    xy = pd.concat([x, y], axis=1)
    #xy_temp = xy.loc[y<y.quantile(0.99)]
    #xy_temp = xy_temp.loc[y<y.quantile(0.8)]
    #xy= xy_temp
...
df_orig = pd.read_csv("/datasets/vowels-unsupervised-ad.csv", header=None)
df_results = pd.read_csv("/final_models/vowels_512bit_500000steps_summary.csv")
...
for method in methods_list:
    ...
    for j in range(1):
        ...
        average_auc += best_auc_final

    #df_main["{}_best_mse".format(method)] = best_feature_mse
    #df_main["{}_best_mse_rank".format(method)] = stats.rankdata(best_feature_mse)
    average_auc_list.append(average_auc/5)
...
df_methods_auc["Average Best AUC"] = average_auc_list
...
#df_main.to_csv('/Users/guyzamberg/PycharmProjects/git/AnomalyDiffusion/final_models/pen_global_df_main_300pt.csv')
