...
df_orig = pd.read_csv("/Users/guyzamberg/PycharmProjects/git/AnomalyDiffusion/datasets/shuttle-unsupervised-ad.csv", header=None)
df_results = pd.read_csv("/Users/guyzamberg/PycharmProjects/git/AnomalyDiffusion/final_models/shuttle_512bit_300000steps_summary.csv")
...
for i in range(49):
    ...
    print("sum Dist{}nn mse: {}".format(i,sum(norm_feature_mse)))
    if mse_sum == 0:
        continue
    if mse_sum < minimum_mse_sum:
        best_auc = auc(feature_mse, labels)
...
for i in range(5):
    ...
    print("sum LOF{}nn mse: {}".format(i,sum(norm_feature_mse)))
    if mse_sum == 0:
        continue
    if mse_sum < minimum_mse_sum:
        best_auc = auc(feature_mse, labels)
...
for i in range(features_num):
    ...
    print("sum f{} mse: {}".format(i,sum(norm_feature_mse)))
    if mse_sum == 0:
        continue
    if mse_sum < minimum_mse_sum:
        best_auc = auc(feature_mse, labels)
