...
class AnomalyLoop:
    def __init__(
        ...
        **kwargs,
        ...
        self.rpts = 50
        self.rpt_ind, self.rpt_weights, self.noise_array = self.sample_anomaly(rpts=self.rpts,min_linspace=0,max_linspace=99)
    ...
    def init_summary_file(self,knn_low,knn_high):
        ...
        isolation = self.data_obj.get_isolation()
        summary_file["Isolation"] = isolation[0]
        for i in range(knn_low-1,knn_high):
            summary_file["Dist{}nn".format(i)]=distances[i]
        #for i in range(knn_low-1,knn_high):
        for i in range(10, 15):
            summary_file["LOF{}nn".format(i)]=lof[i-10]
        return summary_file
    ...
    def create_summary_file(self):
        ...
        print(self.auc(losses,self.data_obj.get_all_labels()))
    ...
    def sample_anomaly(self, rpts, min_linspace, max_linspace):
        """
        Importance-sample timesteps for a batch.
        ...
        indices_np = np.linspace(min_linspace,max_linspace,rpts,dtype=int)
        indices = th.from_numpy(indices_np)
        weights_np = 1 / (len(p) * p[indices_np])
        weights = th.from_numpy(weights_np).float()
        noise = []
